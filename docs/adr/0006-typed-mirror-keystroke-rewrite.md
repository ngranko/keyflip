...
Cursor forced a second lesson: it answers `AXUIElementSetAttributeValue` with `.success` and changes nothing, so the return value cannot be trusted and every write is verified by re-reading the field. It does honour `setRange`, though. So the fallbacks are ordered by how much they assume — write, then **select the target and type over the selection**, then backspace over what the mirror claims is there. Only the last one counts characters, and it runs only from a caret proven collapsed: a refused write can leave its selection behind, where one backspace would eat the whole run and the next N would eat what came before it. The proof is asked for more than once, with a pause between asks. Slack and Zen bridge Accessibility through another process and answer a read that follows a write from before it, so the first readback described the selection the rung above had just left rather than the caret we had restored — the rewrite was abandoned on state that was already correct, and the same trigger a second later always worked. Asking three times back to back did not help: all three landed inside the same few milliseconds of lag.
...
A rewrite keeps the focused Accessibility element and the input revision captured with its reading. Every delayed step and each transition to synthesized keys checks that the same element remains focused and no new typing, click, or observation gap has invalidated the revision. Secure input and marked text stop the transaction too. Cancellation prevents further retries, repairs, and follow. Exhausting recovery attempts now fails without returning to the old snapshot.

Field discovery uses only the focused text field, text area, or combo box. It no longer searches a wrapper for the longest descendant. An ambiguous wrapper declines conversion even if this reduces compatibility with its app.

Before counted deletion after a failed selection, the field must report the exact restored caret position and the expected text immediately before it. A collapsed selection at another position, different text, or unreadable text cannot authorize backspaces. Direct mirror conversion in fields without readable text still depends on the uninterrupted typing session.

Diagnostics record text lengths rather than content. The log is cleared at app startup to remove snippets persisted by older versions, and new logs use owner-only file permissions.
