...
The extent is the mirror's length. An unknown input event or a backspace that empties the mirror ends the session. Neither case authorizes conversion of the pre-existing word. Modified deletion also ends the session because the app may delete a word or a line instead of one character. New character typing can start a new session, limited to that new text.
