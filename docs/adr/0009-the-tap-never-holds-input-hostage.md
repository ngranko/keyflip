...
**Keys the system counts but the tap is never handed mean it is holding them.** The HID layer stamps a keystroke the moment it is pressed, upstream of any tap, so a system that has seen a key in the last two seconds while the tap has seen none for more than two seconds is a tap holding input. It is the only symptom of this fault visible from anywhere, because the callback cannot report a condition whose definition is that the callback is not running — which is why every earlier defence, all of them inside the callback or inside a trust API, missed both incidents. The supervisor checks it every tick and takes the tap away. Secure input withholds keys from every tap by design and is excluded. What the evidence means depends on what the tap may do: a deleting one is holding the keys, a listening one is merely deaf to them — a tap ahead of ours may be deleting them first — and the same sign is still the only reason to replace it.

**A timeout tears the tap down.** Never re-enabled in place. The supervisor builds a fresh one a second or two later, which is also the trust probe. A second timeout inside the minute is a pattern rather than a hiccup, and a tap that may delete stays gone until a person grants the permission again — nothing automatic revives it.

**Only a tap that may delete is ever given up on.** Retiring is a penalty for costing someone their keyboard, and a listening tap cannot: it is rebuilt as often as the evidence says to. Applied to a listening tap the rule bought nothing and cost the app itself — two strikes a minute apart on a listening tap left a five-day-old process with no tap at all, converting nothing, for the rest of its launch, with a menu bar that looked healthy throughout. A missed conversion is an annoyance; so is a silent hour of them.
