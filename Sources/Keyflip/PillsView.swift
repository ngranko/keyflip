...
final class PillsView: MenuControlsView {
    ...
    init(
        ...
        registerControls([trigger, login])
    ...
    func setLaunchOn(_ on: Bool, needsApproval: Bool = false) {
        login.setTitle(needsApproval ? "Approval needed…" : (on ? "On" : "Off"))
    }
...
final class PillButton: HoverView {
    ...
    init(title: String, subtitle: String, action: @escaping () -> Void) {
        ...
        setAccessibilityLabel(subtitle)
